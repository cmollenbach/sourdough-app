...
async function main() {
  try {
    ...
    const durationParam = await prisma.stepParameter.create({
      data: { name: 'Duration (minutes)', type: ParameterDataType.NUMBER, helpText: 'The length of time for this step, in minutes.', defaultValue: '60' }
    });
    const tempParam = await prisma.stepParameter.create({
      data: { name: 'Temperature (°C)', type: ParameterDataType.NUMBER, helpText: 'The target ambient or dough temperature in Celsius.', defaultValue: '24' }
    });
    const numFoldsParam = await prisma.stepParameter.create({
      data: { name: 'Number of Folds', type: ParameterDataType.NUMBER, advanced: true, helpText: 'The total number of stretch and fold sets to perform.', defaultValue: '4' }
    });
    const prefermentContribParam = await prisma.stepParameter.create({
      data: { name: 'Contribution (pct)', type: ParameterDataType.NUMBER, helpText: 'Percentage of total formula flour used in this preferment.', defaultValue: '20' }
    });
    const prefermentHydrationParam = await prisma.stepParameter.create({
      data: { name: 'Hydration', type: ParameterDataType.NUMBER, helpText: 'Hydration percentage of this preferment.', defaultValue: '100' }
    });
    const steamParam = await prisma.stepParameter.create({
      data: { name: 'Steam Duration (min)', type: ParameterDataType.NUMBER, advanced: true, helpText: 'How long to maintain steam in the oven.', defaultValue: '20' }
    });
          ...
          create: [
            {
              ...
              ingredients: { create: [
                { ingredientId: breadFlour.id, amount: 100, calculationMode: IngredientCalculationMode.PERCENTAGE }
              ]}
            },
            {
              order: 2,
        ...
        steps: {
          create: [
            ...
            },
            {
              ...
              ingredients: { create: [
                { ingredientId: walnuts.id, amount: 100, calculationMode: IngredientCalculationMode.FIXED_WEIGHT },
                { ingredientId: raisins.id, amount: 80, calculationMode: IngredientCalculationMode.FIXED_WEIGHT }
              ]}
            },
            {
              order: 5,
