...
// --- Create a recipe ---
router.post("/recipes", authMiddleware_1.authenticateJWT, (0, validation_1.validateBody)(recipeSchemas_1.createRecipeSchema), async (req, res, next) => {
    try {
...
router.get("/recipes", authMiddleware_1.authenticateJWT, async (req, res, next) => {
    try {
...
router.get("/recipes/:id/full", authMiddleware_1.authenticateJWT, async (req, res, next) => {
    const id = Number(req.params.id);
    if (isNaN(id)) {
        return res.status(400).json({ error: "Invalid recipe id" });
    }
    try {
        const recipe = await prisma_1.default.recipe.findFirst({
            where: {
                id,
                active: true,
                OR: [
...
router.get("/recipes/predefined/by-name", authMiddleware_1.authenticateJWT, async (req, res, next) => {
...
// --- Update a recipe ---
router.put("/recipes/:id", authMiddleware_1.authenticateJWT, (0, validation_1.validateParams)(recipeSchemas_1.recipeIdParamSchema), (0, validation_1.validateBody)(recipeSchemas_1.updateRecipeSchema), async (req, res, next) => {
    const id = Number(req.params.id);
    try {
        ...
        const existingRecipe = await prisma_1.default.recipe.findUnique({
        ...
        await prisma_1.default.$transaction(async (tx) => {
            ...
            if (incomingSteps && Array.isArray(incomingSteps)) {
                ...
                for (const stepPayload of incomingSteps) {
                    ...
                    const incomingPvClientIds = incomingPvs.map((pv) => pv.id).filter((pvId) => pvId && pvId > 0); // Only positive IDs
                    ...
                    const incomingIngClientIds = incomingIngs.map((ing) => ing.id).filter((ingId) => ingId && ingId > 0); // Only positive IDs
        ...
        const finalRecipe = await prisma_1.default.recipe.findUnique({
            where: { id },
            include: {
                steps: {
                    orderBy: { order: "asc" },
...
router.delete("/recipes/:id", authMiddleware_1.authenticateJWT, async (req, res, next) => {
    const id = Number(req.params.id);
    if (isNaN(id)) {
        return res.status(400).json({ error: "Invalid recipe id" });
    }
    try {
        const recipe = await prisma_1.default.recipe.updateMany({
...
router.post("/recipes/:id/clone", authMiddleware_1.authenticateJWT, async (req, res, next) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ error: "Invalid recipe id" });
    try {
        ...
        const newRecipe = await prisma_1.default.recipe.create({
            data: {
                ownerId: req.user.userId,
                isPredefined: false,
                // Copy direct fields
