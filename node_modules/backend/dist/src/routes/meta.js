...
router.get("/step-templates", async (_req, res) => {
    const templates = await prisma_1.default.stepTemplate.findMany({
        include: {
...
router.get("/ingredients", async (_req, res) => {
    ...
    const ingredients = await prisma_1.default.ingredient.findMany({ orderBy: { name: "asc" } });
    res.json({ ingredients });
});
// Ingredient categories/groups
router.get("/ingredient-categories", async (_req, res) => {
    // This query remains the same
    const categories = await prisma_1.default.ingredientCategory.findMany({ orderBy: { name: "asc" } });
    res.json({ categories });
});
// Get all StepParameters (formerly Fields)
router.get("/fields", async (_req, res) => {
    // CHANGED: Use the new 'stepParameter' model name
    const stepParameters = await prisma_1.default.stepParameter.findMany({ orderBy: { name: "asc" } });
