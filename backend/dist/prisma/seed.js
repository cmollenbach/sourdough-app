...
async function main() {
    ...
    const durationParam = await prisma.stepParameter.upsert({ where: { name: 'Duration (minutes)' }, update: { helpText: 'The length of time for this step, in minutes. This can vary based on other factors like temperature.' }, create: { name: 'Duration (minutes)', type: client_1.ParameterDataType.NUMBER, helpText: 'The length of time for this step, in minutes. This can vary based on other factors like temperature.', defaultValue: '60' } });
    const tempParam = await prisma.stepParameter.upsert({ where: { name: 'Temperature (°C)' }, update: { helpText: 'The target ambient or dough temperature in Celsius. Crucial for controlling fermentation speed.' }, create: { name: 'Temperature (°C)', type: client_1.ParameterDataType.NUMBER, helpText: 'The target ambient or dough temperature in Celsius. Crucial for controlling fermentation speed.', defaultValue: '24' } });
    const numFoldsParam = await prisma.stepParameter.upsert({ where: { name: 'Number of Folds' }, update: { advanced: true, helpText: 'The total number of stretch and fold sets to perform during bulk fermentation to build dough strength.' }, create: { name: 'Number of Folds', type: client_1.ParameterDataType.NUMBER, advanced: true, helpText: 'The total number of stretch and fold sets to perform during bulk fermentation to build dough strength.', defaultValue: '4' } });
    ...
    const prefermentContribParam = await prisma.stepParameter.upsert({
        ...
        create: { name: 'Contribution (pct)', type: client_1.ParameterDataType.NUMBER, helpText: 'Percentage of total formula flour used in this preferment. E.g., 20 for 20%.', defaultValue: '20', order: 1 } // Added order
    });
    const prefermentHydrationParam = await prisma.stepParameter.upsert({
        ...
        create: { name: 'Hydration', type: client_1.ParameterDataType.NUMBER, helpText: 'Hydration percentage of this preferment (water as % of preferment flour). E.g., 100 for 100%.', defaultValue: '100', order: 2 } // Added order
    });
