...
router.get('/profile', authMiddleware_1.authenticateJWT, async (req, res) => {
    try {
...
router.put('/profile', authMiddleware_1.authenticateJWT, async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
...
router.post('/actions', authMiddleware_1.authenticateJWT, async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
...
router.get('/preferences', authMiddleware_1.authenticateJWT, async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
...
router.put('/preferences', authMiddleware_1.authenticateJWT, async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
        ...
        if (profile && Object.keys(complexPrefs).length > 0) {
            const updates = Object.entries(complexPrefs).map(([key, value]) => prisma.userPreference.upsert({
                where: {
