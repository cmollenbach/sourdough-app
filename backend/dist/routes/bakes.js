...
router.get('/:bakeId', async (req, res) => {
    const ownerId = req.user?.userId;
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ message: 'Failed to create bake.', details: error.message });
    }
});
// PUT /api/bakes/:bakeId/notes: Updates the notes for a specific bake.
router.put('/:bakeId/notes', async (req, res) => {
    const ownerId = req.user?.userId;
    const { bakeId } = req.params;
...
router.put('/:bakeId/rating', async (req, res) => {
    const ownerId = req.user?.userId;
    const { bakeId } = req.params;
...
router.put('/:bakeId/complete', async (req, res) => {
    const ownerId = req.user?.userId;
    const { bakeId } = req.params;
    try {
        const bake = await prisma.bake.findFirst({ where: { id: Number(bakeId), ownerId: ownerId } });
...
router.put('/:bakeId/cancel', async (req, res) => {
    const ownerId = req.user?.userId; // Corrected to use userId
    const { bakeId } = req.params;
    try {
        const bake = await prisma.bake.findFirst({ where: { id: Number(bakeId), ownerId: ownerId } });
...
router.put('/:bakeId/steps/:stepId/start', async (req, res) => {
    const ownerId = req.user?.userId; // Corrected to use userId
...
router.put('/:bakeId/steps/:stepId/complete', async (req, res) => {
    const ownerId = req.user?.userId; // Corrected to use userId
    const { bakeId: bakeIdStr, stepId: stepIdStr } = req.params;
    const bakeId = Number(bakeIdStr);
    const stepId = Number(stepIdStr);
    ...
    try {
        ...
        res.status(500).json({ message: 'Failed to complete step.', details: error.message });
    }
});
// PUT /api/bakes/:bakeId/steps/:stepId/skip: Sets step status to SKIPPED.
router.put('/:bakeId/steps/:stepId/skip', async (req, res) => {
    const ownerId = req.user?.userId; // Corrected to use userId
    const { bakeId: bakeIdStr, stepId: stepIdStr } = req.params;
    const bakeId = Number(bakeIdStr);
    const stepId = Number(stepIdStr);
...
router.put('/:bakeId/steps/:stepId/parameters/:parameterValueId/actual', async (req, res) => {
    const ownerId = req.user?.userId; // Corrected to use userId
...
router.put('/:bakeId/steps/:stepId/note', async (req, res) => {
    const ownerId = req.user?.userId; // Corrected to use userId
    const { bakeId: bakeIdStr, stepId: stepIdStr } = req.params;
    const bakeId = Number(bakeIdStr);
    const stepId = Number(stepIdStr);
...
router.put('/:bakeId/steps/:stepId/deviations', async (req, res) => {
    const ownerId = req.user?.userId;
    const { bakeId: bakeIdStr, stepId: stepIdStr } = req.params;
    const bakeId = Number(bakeIdStr);
    const stepId = Number(stepIdStr);
...
router.put('/:bakeId/steps/:stepId/parameter-values/:parameterValueId/planned', async (req, res) => {
    const ownerId = req.user?.userId;
    const { bakeId: bakeIdStr, stepId: stepIdStr, parameterValueId: bspvIdStr } = req.params;
    const bakeId = Number(bakeIdStr);
    const stepId = Number(stepIdStr);
